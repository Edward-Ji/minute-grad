...
def main():
    ...
    for _ in range(100):
        # linear model
        model_batchnorm = Composite(
            [
                Linear(2, 32),
                ReLU(),
                BatchNormalisation(32),
                Linear(32, 32),
                ReLU(),
                BatchNormalisation(32),
        ...
        optimiser = AdamOptimiser(model_batchnorm.get_all_tensors())
        loss_fn = CrossEntropyLoss()

        model_batchnorm.train(False)
        logits = model_batchnorm(X_test)
        accuracy = np.mean(np.argmax(logits.val, axis=1) == y_test.val)
        print(f"Before training test accuracy: {accuracy:.2f}")
        
        for _ in range(100):
            model_batchnorm.train(True)
            for X_batch, y_batch in batches_train:
                optimiser.zero_grad()
                logits = model_batchnorm(X_batch)
    ...
    print(f'Average of 100 rounds of training with batch norm: {sum(accuracies)/len(accuracies):.2f}')
    print(accuracies)
    accuracies = []
    for _ in range(100):
        # linear model
    ...
    print(f'Average of 100 rounds of training without batch norm: {sum(accuracies)/len(accuracies):.2f}')
    print(accuracies)
